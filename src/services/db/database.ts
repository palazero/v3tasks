...
export class AppDatabase extends Dexie {
  ...
  constructor() {
    ...
    this.version(1).stores({
      ...
      // 任務表
      tasks: 'taskId, projectId, parentTaskId, assigneeId, statusId, priorityId, creatorId, [projectId+parentTaskId], order',
  ...
  async clearAllData(): Promise<void> {
    await this.transaction('rw', this.users, this.projects, this.tasks, this.views, this.customFields, async () => {
      await Promise.all([
        this.users.clear(),
        this.projects.clear(),
        this.tasks.clear(),
