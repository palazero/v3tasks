  ...
  for (const project of projects) {
    ...
    for (let i = 0; i < taskCount; i++) {
      ...
      const task: Task = {
        ...
        statusId: taskStatuses[i % taskStatuses.length],
        assigneeId: assignees[i % assignees.length] || undefined,
        priorityId: taskPriorities[i % taskPriorities.length],
...
function getRandomTags(): string[] {
  const allTags = ['前端', '後端', '設計', '測試', '文檔', '緊急', '重要', 'API', 'UI/UX', '資料庫']
