...
export class ViewConfigurationService {
  ...
  private async initializeTables(): Promise<void> {
    ...
    if (!db.viewConfigurations) {
      await databaseService.addTable('viewConfigurations', '++id, userId, projectId, viewType, configuration, createdAt, updatedAt')
    }
    
    // 確保視圖預設表存在
    if (!db.viewPresets) {
      await databaseService.addTable('viewPresets', '++id, name, description, configuration, isSystem, createdBy, createdAt, updatedAt')
    }
  }
   ...
   * 取得用戶所有視圖配置
   */
  async getUserViewConfigurations(userId: string): Promise<Array<{
    projectId: string
    viewType: string
    configuration: ViewConfiguration
    updatedAt: Date
    ...
    return await db.viewConfigurations
      .where('userId')
      .equals(userId)
      .toArray()
  }
   ...
   * 儲存視圖預設配置
   */
  async saveViewPreset(preset: Omit<ViewPreset, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    const db = databaseService.getDatabase()
  ...
  async getViewPresets(includeSystem = true): Promise<ViewPreset[]> {
    ...
    if (!includeSystem) {
      query = query.filter(preset => !preset.isSystem)
    }
  ...
  async initializeSystemPresets(): Promise<void> {
    ...
    // 檢查是否已有系統預設
    const existingSystemPresets = await db.viewPresets
      .where('isSystem')
      .equals(true)
      .count()
