...
export function useTaskDependencies() {
  ...
  function getBlockedTasks(task: Task, allTasks: Task[]): Task[] {
    return allTasks.filter(t => 
      t.dependencyIds && t.dependencyIds.includes(task.taskId)
    )
  }
   ...
   * 新增任務依賴關係
   */
  function addTaskDependency(taskId: string, dependencyId: string, allTasks: Task[]): {
    valid: boolean
   ...
   * 移除任務依賴關係
   */
  function removeTaskDependency(taskId: string, dependencyId: string, allTasks: Task[]): {
    valid: boolean
    updates?: Array<{ taskId: string; updates: Partial<Task> }>
    error?: string
  } {
   ...
   * 檢查是否會造成循環依賴
   */
  function wouldCreateCircularDependency(taskId: string, dependencyId: string, allTasks: Task[]): boolean {
   ...
   * 取得任務的依賴狀態
   */
  function getTaskDependencyStatus(task: Task, allTasks: Task[]): {
    canStart: boolean
    blockedBy: Task[]
    dependsOn: Task[]
    blocking: Task[]
    ...
    // 計算前置任務完成率
    const completionRate = dependsOn.length === 0 ? 1 : 
   ...
   */
  function autoUpdateTaskStatus(completedTaskId: string, allTasks: Task[]): Array<{ taskId: string; updates: Partial<Task> }> {
    const updates: Array<{ taskId: string; updates: Partial<Task> }> = []
    const blockedTasks = getBlockedTasks(allTasks.find(t => t.taskId === completedTaskId)!, allTasks)
