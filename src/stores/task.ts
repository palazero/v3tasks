...
export const useTaskStore = defineStore('task', () => {
  ...
  // Composables
  const { buildTaskTree, createSubtask, indentTask, outdentTask, toggleTaskExpanded: toggleExpanded } = useNestedTasks()
  const { addTaskDependency, removeTaskDependency, getTaskDependencyStatus } = useTaskDependencies()
  ...
  async function assignTask(taskId: string, assigneeId?: string): Promise<boolean> {
    const updateData: Partial<Task> = { assigneeId: assigneeId || undefined }
  ...
  async function addSubtask(parentTask: Task, title: string): Promise<Task | null> {
    ...
    try {
      ...
      const newSubtask = await taskRepo.create(subtaskData)
  ...
  // 批量更新任務（用於拖拉排序）
  async function batchUpdateTasks(updates: Array<{ taskId: string; updates: Partial<Task> }>): Promise<boolean> {
    isLoading.value = true
    error.value = null

    try {
      await Promise.all(updates.map(update => 
        taskRepo.update(update.taskId, update.updates)
      ))
  ...
  // 移除任務依賴
  async function removeTaskDependencyAction(taskId: string, dependencyId: string): Promise<boolean> {
